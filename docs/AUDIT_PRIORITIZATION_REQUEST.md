...
### 主题一：平台的三件"验证级"宣称，有两件站不住

BUG-21（walk-forward OOS 含训练期）、BUG-22（基本面提前可见）、BUG-23（ML CV 按行切分）
合起来意味着：**这个平台用来证明"我的因子/模型有效"的那套工具，本身是有偏的。**

这对一个把"诚实研究 / Truth First"写进架构文档的项目，比任何单个 bug 都严重 ——
因为它不是"某个功能坏了"，而是"我们用来判断功能好坏的那把尺子坏了"。
